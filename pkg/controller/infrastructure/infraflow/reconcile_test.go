...
var _ = Describe("getSubnetKey", func() {
	...
	newFC := func(ipFamilies []v1beta1.IPFamily, zones []awsapi.Zone, stateSubnets map[string]string) *FlowContext {
		...
		zoneChild := state.GetChild(ChildIdZones).GetChild(zone)
	...
	ipv4Zones := []awsapi.Zone{{Name: zone, Workers: ptr.To("10.0.0.0/19"), Public: ptr.To("10.0.48.0/20"), Internal: ptr.To("10.0.112.0/22")}}
	byoZones := []awsapi.Zone{{Name: zone, WorkersSubnetID: ptr.To("subnet-workers"), PublicSubnetID: ptr.To("subnet-public"), InternalSubnetID: ptr.To("subnet-internal")}}
	ipv6Zones := []awsapi.Zone{{Name: zone}}
	...
	DescribeTable("resolves subnet key",
		...
		// IPv4: CIDR-based matching
		Entry("IPv4 workers subnet matched by CIDR",
			ipv4Only, ipv4Zones, nil,
			&awsclient.Subnet{SubnetId: "subnet-aaa", CidrBlock: "10.0.0.0/19", AvailabilityZone: zone},
			IdentifierZoneSubnetWorkers, false),
		Entry("IPv4 public subnet matched by CIDR",
			ipv4Only, ipv4Zones, nil,
			&awsclient.Subnet{SubnetId: "subnet-bbb", CidrBlock: "10.0.48.0/20", AvailabilityZone: zone},
			IdentifierZoneSubnetPublic, false),
		Entry("IPv4 internal subnet matched by CIDR",
			ipv4Only, ipv4Zones, nil,
			&awsclient.Subnet{SubnetId: "subnet-ccc", CidrBlock: "10.0.112.0/22", AvailabilityZone: zone},
			IdentifierZoneSubnetPrivate, false),
		Entry("IPv4 unknown CIDR returns error",
			ipv4Only, ipv4Zones, nil,
			&awsclient.Subnet{SubnetId: "subnet-xxx", CidrBlock: "10.99.0.0/24", AvailabilityZone: zone},
			"", true),
		// BYO dual-stack: ID-based matching from state
		Entry("BYO dual-stack workers subnet matched by ID",
			dualStack, byoZones, byoState,
			&awsclient.Subnet{SubnetId: "subnet-workers", AvailabilityZone: zone},
			IdentifierZoneSubnetWorkers, false),
		Entry("BYO dual-stack public subnet matched by ID",
			dualStack, byoZones, byoState,
			&awsclient.Subnet{SubnetId: "subnet-public", AvailabilityZone: zone},
			IdentifierZoneSubnetPublic, false),
		Entry("BYO dual-stack internal subnet matched by ID",
			dualStack, byoZones, byoState,
			&awsclient.Subnet{SubnetId: "subnet-internal", AvailabilityZone: zone},
			IdentifierZoneSubnetPrivate, false),
		Entry("BYO dual-stack unknown subnet ID returns error",
			dualStack, byoZones, byoState,
			&awsclient.Subnet{SubnetId: "subnet-unknown", AvailabilityZone: zone},
			"", true),
		// IPv6-only (Gardener-managed): ID-based matching from state
		Entry("IPv6-only workers subnet matched by ID from state",
			ipv6Only, ipv6Zones, ipv6State,
			&awsclient.Subnet{SubnetId: "subnet-workers-ipv6", AvailabilityZone: zone},
			IdentifierZoneSubnetWorkers, false),
		Entry("IPv6-only public subnet matched by ID from state",
			ipv6Only, ipv6Zones, ipv6State,
			&awsclient.Subnet{SubnetId: "subnet-public-ipv6", AvailabilityZone: zone},
			IdentifierZoneSubnetPublic, false),
		Entry("IPv6-only internal subnet matched by ID from state",
			ipv6Only, ipv6Zones, ipv6State,
			&awsclient.Subnet{SubnetId: "subnet-internal-ipv6", AvailabilityZone: zone},
			IdentifierZoneSubnetPrivate, false),
		// IPv6-only (Gardener-managed): name tag matching (fallback when ID not in state)
		Entry("IPv6-only workers subnet matched by name tag",
			ipv6Only, ipv6Zones, nil,
			&awsclient.Subnet{SubnetId: "subnet-workers-ipv6", AvailabilityZone: zone, Tags: awsclient.Tags{TagKeyName: ns + "-nodes-" + suffix}},
			IdentifierZoneSubnetWorkers, false),
		Entry("IPv6-only public subnet matched by name tag",
			ipv6Only, ipv6Zones, nil,
			&awsclient.Subnet{SubnetId: "subnet-public-ipv6", AvailabilityZone: zone, Tags: awsclient.Tags{TagKeyName: ns + "-public-utility-" + suffix}},
			IdentifierZoneSubnetPublic, false),
		Entry("IPv6-only internal subnet matched by name tag",
			ipv6Only, ipv6Zones, nil,
			&awsclient.Subnet{SubnetId: "subnet-internal-ipv6", AvailabilityZone: zone, Tags: awsclient.Tags{TagKeyName: ns + "-private-utility-" + suffix}},
			IdentifierZoneSubnetPrivate, false),
